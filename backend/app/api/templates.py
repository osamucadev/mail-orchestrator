...
def read_templates(db: Session = Depends(get_account_db)):
    return list_templates(db)


@router.post("", response_model=TemplateRead, status_code=status.HTTP_201_CREATED)
def write_template(payload: TemplateCreate, db: Session = Depends(get_account_db)):
    return create_template(db, payload.model_dump())


@router.get("/{template_id}", response_model=TemplateRead)
def read_template(template_id: int, db: Session = Depends(get_account_db)):
...
def edit_template(template_id: int, payload: TemplateUpdate, db: Session = Depends(get_account_db)):
...
def remove_template(template_id: int, db: Session = Depends(get_account_db)):
...
def read_placeholders(template_id: int, db: Session = Depends(get_account_db)):
